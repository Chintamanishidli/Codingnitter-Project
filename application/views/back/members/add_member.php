...
<div id="content-container">
	...
	<div id="page-content">
		...
		// Get the admin info (including role_id)
		$admin = $this->db->get_where("admin", ["admin_id" => $admin_id])->row();

		// Get the admin area info
		$admin_area = $this->db->get_where("admin_area", ["admin_id" => $admin_id])->row();
		...
		<h1 class="btn btn-success" disabled>
			<?= $this->session->userdata('role_name') ?? "default role"; ?>
		</h1>
			...
			<div class="panel-body">
				<form class="form-horizontal" id="manage_details_form" method="POST" action="<?= base_url() ?>admin/members/add_member/do_add">
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="form-control" value="<?php if (!empty($form_contents)) {
																																echo $form_contents['fname'];
																															} ?>" name="fname" placeholder="<?= translate('first_name') ?>">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="form-control" value="<?php if (!empty($form_contents)) {
																																echo $form_contents['lname'];
																															} ?>" name="lname" placeholder="<?= translate('last_name') ?>" required="">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="form-control" value="<?php if (!empty($form_contents)) {
																															...
																															} ?>" name="email" placeholder="<?= translate('email') ?>" required="">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="date" value="<?php if (!empty($form_contents)) {
																					echo $form_contents['date_of_birth'];
																				} ?>" class="form-control" name="date_of_birth" style="line-height: normal;">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label" for="mobile"><b><?= translate('mobile') ?> <span class="text-danger">*</span></b></label>
						<div class="col-sm-8">
							<input type="number" value="<?php if (!empty($form_contents)) {
																						echo $form_contents['mobile'];
																					} ?>" class="form-control" name="mobile" placeholder=" <?= translate('mobile_no.') ?>">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<select id="area" name="area_id" class="form-control" disabled>
								<?php foreach ($areas as $area): ?>
									<option value="<?= $area['id'] ?>" <?= ($area['id'] == $admin_area->area_id) ? 'selected' : '' ?>>
										<?= $area['name'] ?>
									</option>
								<?php endforeach; ?>
							</select>
							<input type="hidden" name="area" id="area_name" value="<?= $admin_area->area_id ?? '' ?>">
						</div>
					</div>
