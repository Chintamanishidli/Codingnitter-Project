...
class Admin extends CI_Controller
{
	...
	public function add_legion()
	{
		...
		// Get POST data - FIXED: match form field names
		$legion_name = $this->input->post('legion_name');
		$area_id     = $this->input->post('area_id');
		$short       = $this->input->post('short'); // FIXED: match form field name

		// Log the received data - FIXED: use correct variable names
		log_message('info', 'Add Legion request received with data: name=' . $legion_name . ', area_id=' . $area_id . ', short=' . $short);

		// Simple validation - FIXED: use correct variable names
		if (empty($legion_name) || empty($area_id) || empty($short)) {
			$response = [
				'success' => false,
				'message' => 'Legion name, short name, and area ID are required.'
			];
		...
		// Prepare data
		$data = [
			'name'    => $legion_name,  // FIXED: use correct variable
			'short'   => $short,
			'area_id' => $area_id
		];
		...
		if ($insert_id) {
			$response = [
				...
				'legion_short'  => $short,      // FIXED: consistent naming
				'area_id'       => $area_id
			];
	...
	function manage_admin()
	{
		if ($this->admin_permission() == FALSE) {
			...
			// $page_data['role_name'] = $this->session->userdata('role_name') ?? "Nn";
	...
	function check_login()
	{
		if ($this->admin_permission() == TRUE) {
			...
			if ($result) {
				// Get role name from role table using role_id
				$role_name = $this->Crud_model->get_role_name($result->role);

				// Set session data
				$data = array(
					'admin_name' => $result->email,
					'admin_id'   => $result->admin_id,
					'role_id'    => $result->role,
					'role_name'  => $role_name ? $role_name : 'No Role Assigned',
					'user_type'  => 1
				);
	...
	// 	// Example: fetch area/legion info by ID
	// 	$info = $this->Crud_model->get_area_by_id($id);



	// 	$data['area_id'] = $info->area_id;
	// 	$this->load->view('area_page', $data);
	//}
	...
	public function show_role()
	{
		...
		$data['info'] = $this->Crud_model->get_role_by_admin($admin_id);

		$this->load->view('display_role', $data);
	}
